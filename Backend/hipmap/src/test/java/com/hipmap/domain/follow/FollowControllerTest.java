...
class FollowControllerTest {
    ...
    /* @Mock어노테이션을 붙여서 선언된 followService는
    ...
    void delete_follow() throws Exception {
        ...
        /* followService.deleteFollow() 호출되면
         * list 객체가 리턴되도록 설정 */
        when(followService.deleteFollow(userId, opponentUserId)).thenReturn(1L);

        /* "/follow/opponentUserId" 경로를 DELETE 방식으로 호출하기 위한
         * 경로 정보를 가지고 있는 reqBuilder객체를 생성   */
        RequestBuilder reqBuilder = MockMvcRequestBuilders
                .delete("/follow/" + opponentUserId).contentType(MediaType.APPLICATION_JSON);
        // then
        // reqBuilder에 해당하는 URL을 호출한 후, 상태 코드가 200일 경우 성공합니다. 그리고 결과를 출력
        mvc.perform(reqBuilder).andExpect(status().isOk()).andDo(print());

        /* followService Mock 객체의 delete(userId,opponentUserId)
         * 메소드가 Web API가 동작하면서 호출되었다면 성공 */
        verify(followService).deleteFollow(userId,opponentUserId);
